...
float vertices[] = {		//ZAMIENIAM WSPOLRZEDNE 1 RYSUNKU NA TROJKAT
	-2.8, 2, 0,
	0, -3.5, 0,
	2.8, 2, 0,
};
...
float textureCoords[] = {		//USTAWIAM TEKSTURE 1 RYSUNKU
	0,1,
	0.5,0,
	1,1
 };
...
GLuint  tex_id0;
GLuint  tex_id1;		//TWORZENIE ZMIENNEJ TEKSTURY DLA 2 RYSUNKU
...
void rysuj(void)
{
	...
	MV = glm::translate(MV, glm::vec3(-4, 0, kameraD + dx));		//PRZESUWAM  1 RYSUNEK W LEWO O -4
	...
	glVertexAttrib3f(2, 1.0, 0.0, 0.8);				//FIOLETOWY
	...
	glDrawElements(GL_TRIANGLES, 3, GL_UNSIGNED_INT, 0);	//GL_TRIANGLES RYSUJE TROJKAT


	MV = glm::translate(MV, glm::vec3(6.2, 0, 0));	//PRZESUWAMY SIE W PRAWO ROBIAC MIEJSCE NA RYSUNEK 2
	MVP = P * MV;
	...
	glBindVertexArray(VAO[0]);
	glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
	glVertexAttrib3f(2, 0.0, 0.0, 1.0);				//NIEBIESKI
...
int main(int argc, char **argv)
{
	...
	glutCreateWindow("zajecia 7 cwiczenie 4");
   ...
   tex_id0 = WczytajTeksture("monalisa.bmp");
	...
	tex_id1 = WczytajTeksture("3.bmp");		//WCZYTANIE TEKSTURY DLA 2 RYSUNKU
	if (tex_id1 == -1)
	{
		MessageBox(NULL, "Nie znaleziono pliku z teksturą", "Problem", MB_OK | MB_ICONERROR);
		exit(0);
	...
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D, tex_id1);
	uniformTex0 = glGetUniformLocation(programID, "tex1");
	glUniform1i(uniformTex1, 0);							//PRZYPISANIE ZMIENNEJ tex_id1 DO TEKSTURY Z 3.bmp
