...
# Wczytaj dane
housing <- read.csv("mieszkania.csv")

# Wyświetl pierwsze rekordy
head(housing)

# Wybierz tylko zmienne numeryczne i usuń NA
housing_num <- housing %>%
...
reverse_order <- rev(names(housing_num))
stats <- stats %>%
...
# Macierz korelacji
cor_matrix <- round(cor(housing_num), 2)
print(cor_matrix)
...
# 🔴 Wyklucz zmienną celu z PCA
housing_pca <- housing_num %>%
  select(-wartosc_mediana)

# Normalizacja danych
housing_scaled <- scale(housing_pca)

# PCA - macierz składowych głównych
pca_result <- prcomp(housing_scaled, center = TRUE, scale. = TRUE)
