...
void rysuj(void)
{
	...
	MV = glm::translate(MV, glm::vec3(2.0, 0.0, kameraD));			//PRZESUNIECIE UKLADU WSPOLRZEDNYCH 2.0 W PRAWO
	...
	glVertexAttrib3f(1, 1.0, 0.0, 0.0);					// GOLOR CZEROWNY PRAWEJ FIGURY
	glLineWidth(5);										// GRUBOSC RYSOWANEJ LINII
	glDrawArrays(GL_TRIANGLES, 0, 6);					// 6 PUNKTOW RYSOWANYCH METODA GL_TRIANGLES


	MV = glm::translate(MV, glm::vec3(-1.0, 0.0, 0));		// TO JEST SRODKOWA FIGURA (PRZESUNIETA 0 1.0 W LEWO)
	MVP = P * MV;

	glUniformMatrix4fv(MVP_id, 1, GL_FALSE, &(MVP[0][0]));
	glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
	glVertexAttrib3f(1, 1.0, 0.0, 0.0);						// KOLOR CZERWONY SRODKOWEJ FIGURY
	glDrawArrays(GL_TRIANGLES, 0, 6);						// 6 PUNKTOW RYSOWANYCH METODA GL_TRIANGLES

	
	MV = glm::translate(MV, glm::vec3(-1.0, 0.0, 0));		// FIGURA LEWA PRZESUNIETA O 1.0 W LEWO WZGLEDEM SRODKOWEJ
	MVP = P * MV;

	glUniformMatrix4fv(MVP_id, 1, GL_FALSE, &(MVP[0][0]));
	glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
	glVertexAttrib3f(1, 1.0, 0.0, 0.0);						// CZERWONY KOLOR LEWEJ FIGUTY
	glDrawArrays(GL_TRIANGLES, 0, 6);						// 6 PUNKTOW RYSOWANYCH METODA GL_TRIANGLES

	MV = glm::translate(MV, glm::vec3(-1.0, 0.0, 0));
	MVP = P * MV;

	MV = glm::rotate(MV, (float)glm::radians(dx), glm::vec3(1.0, 0, 0));		// FUNKCJA GLM::ROTATE ODPOWIADA ZA OBROT 
	//MV = glm::translate(MV, glm::vec3(dx, 0, 0));								// KOMENT, BO CHCEMY OBRACAC A NIE PRZESUWAC
	MVP = P * MV;

	glUniformMatrix4fv(MVP_id, 1, GL_FALSE, &(MVP[0][0]));
	glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);				// GL_FILL TO OPCJA ZAPELNIANIA TLA WEWNATRZ GL_TRIANGLES W RUSZAJACEJ SIE FIGURZE
	glVertexAttrib3f(1, 0.5, 0.5, 0.5);						// SZARY SRODEK RUSZAJCEJ SIE FIGURY
	glDrawArrays(GL_TRIANGLES, 0, 6);						// 6 PUNKTOW RYSOWANYCH METODA GL_TRIANGLES

	
	glUniformMatrix4fv(MVP_id, 1, GL_FALSE, &(MVP[0][0]));
	glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
	glVertexAttrib3f(1, 1.0, 1.0, 0.0);						// ZIELONA OBWODKA RUSZAJACEJ SIE FIGURY
